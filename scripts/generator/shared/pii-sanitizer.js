...
// street-level 住所: 市区町村 + 任意の町名 + （丁目/番地/号 または ハイフン連結数字）。
// 「東京都千代田区外神田３丁目６番５号」「千代田区外神田3-6-5」
// 「東京都　千代田区　外神田　３－６－５－８０５」（gBiz の事業所テーブル・全角空白区切り）等にマッチ。
// 「東京都内」「千代田区の企業」「港区エリア」等（番地なし）にはマッチさせない。
const WS = "[ \\t　]*";
const SEP = "(?:丁目|番地の|番地|番|号|[-－ー−‐~〜])";
const STREET_ADDRESS = new RegExp(
  `(?:${PREF})?${WS}` +
    `[一-龥ぁ-んァ-ヶーА-я\\w]{1,8}?[市区町村]${WS}` +
    `(?:[一-龥ぁ-んァ-ヶー]{1,10}?)?${WS}` +
    `${N}{1,4}${WS}${SEP}${WS}${N}{1,4}(?:${WS}${SEP}${WS}${N}{1,4})*${WS}号?`,
  "g"
);
