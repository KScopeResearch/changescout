...
- **`docs/strategy/` change reporting rule**: since everything under `docs/strategy/` (including `PROJECT.md`, `ROADMAP.md`, `FINAL_MVP_PLAN.md`) is untracked/private and never shows up in `git status` or `git diff`, always report the content and reason for any change made there — the user has no other way to see it without opening the file themselves.
